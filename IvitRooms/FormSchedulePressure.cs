...
namespace IvitRooms
{
    public partial class FormSchedulePressure : Form
    {
        ...
        protected List<Schedule> PressuresSchedule = new List<Schedule>();

        protected UserJConfig userJconfig = UserJConfig.Inst();

        public FormSchedulePressure()
        {
            InitializeComponent();
        }

        private void FormSchedulePressures_Load(object sender, EventArgs e)
        {
            ...
            chart2.Titles.Add(new System.Windows.Forms.DataVisualization.Charting.Title("Перепад"));
            this.SetTreeVuew1();
            if (checkBox1.Checked == true)
            {
                this.SetPressuressSchedulePeriod();
            }
            else
            {
                this.SetPressuressSchedule();
            }
            this.SetListViev1();
            this.SetChart2();
        ...
        private void SetChart2()
        {
            ...
            foreach (Schedule ivit in PressuresSchedule)
            {
                Int32 PK = 0;
                Form1._form._newRoot.pases.ForEach(p =>
                {
                    if (p.id == ivit.id)
                    {
                        PK = p.pressurePK;
        ...
        {
            userJconfig.formSchedulePressures.checkedPases.Clear();
            foreach (ListViewItem item in listView1.CheckedItems)
            {
                userJconfig.formSchedulePressures.checkedPases.Add(Convert.ToInt32(item.Name));
            }
            userJconfig.Save();
